...
### 1.1 SkillHub 专项运营（v1.1.0 审核中，优先级最高）
...
- [x] **改名称**：`馒头 GEO 投稿` → `馒头新闻发布`（v1.1.0 包已提交；公开页待审核后更新）
...
- [x] **流程顺序调整**：v1.1.0 先做免 key search，再引导安装、首篇发布和再次发布；搜索/查询仍免鉴权。
...
### 1.2 内容标题与摘要试行（2026-09-09）

按「短标题 + 事实摘要」更新 5 篇真实文章，保留 external_id 与原始来源链接，随后用公开搜索逐篇回查：

| 内容 | 搜索验证 |
|---|---|
| Tencent EVIE | `EVIE` 命中 2 条，首条为更新后的标题 |
| ACE | `ACE` 命中 3 条，首条为更新后的标题 |
| Molecular Déjà Vu | 精确短语命中 1 条 |
| MiniCPM5-2B | 精确实体命中 1 条 |
| CUA-Universe | 精确实体命中 1 条 |

统计接口 `GET /v1/stats/daily?days=30` 会持续记录 HTTP/MCP 搜索、搜索曝光、搜索点击、首页点击和文章浏览；下一周按这些聚合值比较曝光、点击率与有效收录。
