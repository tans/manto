...
import { recordSearch, type SearchChannel } from "./analytics";
function toFtsQuery(raw: unknown): string {
  const cleaned = String(raw || "").trim().replace(/"/g, '""');
  if (!cleaned) throw new Error("query_required");
  return `"${cleaned}"`;
}
export function search(input: any, channel: SearchChannel = "http") {
  ...
  recordSearch(channel, [...(sponsored ? [sponsored.content_id] : []), ...results.map(result => result.content_id)]);
